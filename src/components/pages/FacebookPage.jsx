...
import facebookLoader from "./pages/Facebook.json";
...
console.log("API_BASE:", API_BASE);
  ...
  const fetchInfo = async () => {
    ...
    try {
      const res = await axios.post(`${API_BASE}/facebook/info`, { url });
      if (res.data?.success) {
        setInfo(res.data);
      } else {
      ...
      <Paper className="glass-panel fb-form">
        ...
        {loading && (
          <Box sx={{ display: "flex", justifyContent: "center", alignItems: "center", mt: 3 }}>
            <Lottie animationData={facebookLoader} style={{ width: 150, height: 150 }} loop />
          </Box>
        )}
        {error && (
